...
// #include <raytracing/helper_math.h>
...
//     std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> ray_intersection(const torch::Tensor& ray_o, const torch::Tensor& ray_d);
...
//     float3 origin_centered = ray_o - sphere_center_f3;
//     float a = dot(ray_d, ray_d); // if the directions are normalized this is only ones so maybe it's not really needed
//     float b = 2.0 * dot(origin_centered, ray_d);
...
//     float3 point_intersection_t0 = ray_o + t0 * ray_d;
//     float3 point_intersection_t1 = ray_o + t1 * ray_d;
