...
// Sphere::Sphere(float radius, Eigen::Vector3f center) : m_radius(radius),
//                                                        m_center(center)
// {
...
// Sphere::ray_intersection(const torch::Tensor &rays_o, const torch::Tensor &rays_d)
// {

//     CHECK(rays_o.dim() == 2) << "rays_o should have dim 2 correspondin to nr_raysx3. However it has sizes" << rays_o.sizes();
//     CHECK(rays_d.dim() == 2) << "rays_d should have dim 2 correspondin to nr_raysx3. However it has sizes" << rays_d.sizes();
